...
function stripPositions(node: Node): unknown {
...
function structure(tree: Root): unknown {
...
describe.each(fixtures)('$name', ({ source }) => {
  ...
  it('keeps the document structure through a serialize/parse cycle', () => {
    ...
    expect(structure(parse(serialize(tree)))).toEqual(structure(tree))
  })
  ...
  it('survives a second full cycle unchanged', () => {
    ...
    expect(structure(parse(twice))).toEqual(structure(parse(once)))
  })
})
