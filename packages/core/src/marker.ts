 ...
 * device's provenance.
 */
export function buildMarker(input: BuildMarkerInput): Marker {
  const createdBy: MarkerCreatedBy = {
    appVersion: input.appVersion,
    provider: input.provider,
    clientId: input.clientId,
  }
  if (input.userAgent !== undefined) createdBy.userAgent = input.userAgent
 ...
 * docs/PLAN.md §3.
 */
export function parseMarker(text: string): MarkerParseResult {
  let raw: unknown
  try {
    raw = JSON.parse(text)
  } catch {
