...
- [x] `www.googleapis.com` in the CSP's `connect-src` (§9), with `tests/csp.test.ts` reading a note through the adapter — metadata and bytes both come from it, so it is the one host
