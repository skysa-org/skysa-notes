...
- **Google (Phase 4, `apps/api/src/oauth/gdrive.ts`)** follows the Microsoft shape, with Google's differences:
  - The request carries `access_type=offline` (no refresh token otherwise) and `prompt=consent select_account`. Consent is asked for again because a reconnect gets no refresh token without it; `select_account` is there for the shared-machine reason §5.2 gives. There is no `include_granted_scopes`, so a storage grant never merges with a later sign-in grant.
  - **Granular consent:** Google lets the user untick Drive and still completes the grant. The exchange checks the response's `scope` for `drive.file`. If it is missing, the grant is revoked at once and the user is sent back with `connect=partial`, which the web app answers with "leave that permission ticked". This is not logged, since it is the user's choice.
  ...
  - Disconnecting revokes: `POST oauth2.googleapis.com/revoke` with the freshly refreshed access token, which Google says revokes the refresh token with it.
...
- [x] API: Google storage OAuth (connect with offline access and fresh consent, an unticked Drive scope answered as `connect=partial` and given back, refresh keeping the stored token, revoke on disconnect) in the provider registry (§6, "Storage OAuth, as built")
