...
- Known gaps. Graph says to apply a round's changes after its last page; the engine commits page by page, so a folder deleted on one page and a note moved out of it on the next deletes the clean note here, and — since the move names only the folder it was in — nothing brings it back until a rescan (the file is untouched remotely; a dirty note becomes a conflict copy). A `410` with `resyncChangesUploadDifferences` asks the client to upload what the server did not return; both resync codes are one `CursorResetError`, and the engine's rescan deletes clean notes the scan did not return — after a server-side restore, the notes the restore lost. A folder moved *into* the app folder from elsewhere is reported alone; whether Graph lists what is inside it is one of the live checks.
