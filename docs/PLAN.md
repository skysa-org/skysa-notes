...
- **A remote folder delete keeps the dirty notes inside it**, detaching them so the next push re-creates them, and takes the clean ones and every subfolder with it. The folder is the user's remote layout; the note is their writing, and losing the second to a change in the first is not a trade worth making. The store does the cascade in one transaction, because a provider that reports only the folder gives the engine nothing else to send.
...
- **A push that finds nothing at the path asks whether the file merely moved.** `write` is addressed by path, so a file renamed remotely reports exactly what a deleted one reports, and §7's answer to a deleted file — re-create it — would leave the user with two notes where they had one. The engine reads by `remoteId` first and, if the file is still there, leaves the op queued for the next pull to rebase.
- **A transient pull failure is a status, not a rejection.** Offline, a 500, a store that could not commit: the batch rolled back and the cursor did not move, so `pull` answers `retry` exactly as `push` does. Rejecting instead would make every caller wrap `sync()` in a `try` to discover something `status` exists to tell them, and one that forgot would take the app down on a flight.
