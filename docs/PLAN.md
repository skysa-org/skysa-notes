...
- **The note's revision.** `outsideRevision` counts how many times its body has been replaced from outside the editors: a pull, an import, or the remote side of a conflict. A change that leaves the body alone doesn't count, so a pulled tag or title change, or our own push completing, is not a conflict.
- **What an edit carries.** Each editor remembers the revision of the body it holds: the one it opened with, or the last one it adopted (`useIncomingBody`). Every edit carries that revision.
- **Autosave never merges edits from different revisions.** An edit typed before a pull is saved on its own, before an edit typed into the pulled text replaces it.
- **Same revision:** the save is written as usual.
- **Newer revision** (the body was replaced): the edit becomes a conflict copy beside the note, which keeps the pulled text. If the text is identical, nothing is written.
- **Note gone** (deleted by a pull): the note comes back as it was last shown. It holds the edit, is dirty, has no `remoteId`, and has a `write` queued, so it is re-created on push like any dirty note deleted remotely. If a pulled file has taken its path, it gets a free name.
- **Note deleted here:** the edit is written into the tombstone, which stays deleted, whatever the revision.
- **Known gap:** a row gone because another tab deleted it and its delete was already pushed and purged, all inside the autosave window, looks the same as a remote delete, so the note comes back. That errs toward keeping the words.
