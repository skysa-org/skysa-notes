...
- Root: a folder `APP_FOLDER_NAME` (mimeType `application/vnd.google-apps.folder`) at the top of My Drive, tagged `appProperties: { notesapp: "root" }` and found by that tag, never by name, so a user who renames or moves it keeps their notes. Made when the search finds none (first connect, or the folder is in the trash), then searched again: two devices connecting at once each make one, and both settle on the earliest-made, the later deleting its own (it is new and empty). The id is found afresh at the start of every `changes` call; one that no longer matches the cursor's, or a feed that names the folder trashed or removed, is `CursorResetError`.
...
- Nothing on Drive is conditional: v3 has no `If-Match` and nothing like create-if-absent.
  - Create: look up the name (taken → `ConflictError`, parent missing → `NotFoundError`), upload with `POST /upload/drive/v3/files?uploadType=multipart` (`text/markdown`, bytes exactly as given), then **search again**: if another device's file of that name was made earlier, ours is deleted outright and `ConflictError` names theirs, so the engine makes its conflict copy. A folder is made the same way, and one that exists already is returned.
  ...
  - Move: `PATCH files/{id}` with `{ name }`, and `addParents`/`removeParents` only when the folder changes. The destination is looked up first, so a replayed move returns without a request.
...
- Known gaps. The engine commits page by page, where the feed means a round — the gap §5.2 describes, and Phase 6's item answers for both. A search that misses the existing app folder at a reset makes a second, empty one, and the scan under it deletes this device's clean notes until a later round finds the earlier folder and resets back to it (the files are untouched remotely; the extra folder stays in the user's Drive, empty). Files the user adds through the Drive UI are invisible under `drive.file`.
