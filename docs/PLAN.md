...
- [ ] A pull that hard-deletes a clean note while an autosave is pending makes that save throw `No note with id` (`applyEdit`, `apps/web/src/store/notes.ts`; `NoteView` discards the promise), and the words are lost with no conflict copy — against "never lose user data". Carried from the `useNote` note above; live since the scheduler runs in the browser.
