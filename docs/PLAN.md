...
- **A locally deleted note keeps its row until the remote copy is gone.** The queued `delete` carries a note id and nothing else; the `remoteId` it needs lives on the row. A store that dropped the row the moment the user pressed delete would have the engine find nothing to send, finish the op, and leave the file on the remote for ever — with every test still green.
