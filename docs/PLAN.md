...
- **A file can be gone by the time we read it.** `changes` and `read` are separate round trips on every provider, so a file the feed named can be deleted in between — the other device editing a note and then deleting it inside one cursor window is enough. The entry is skipped rather than thrown on: unwinding the pull leaves the cursor where it was, so the next attempt fetches the same batch and dies the same way, and push never runs either because `sync` stops on a pull that is not `ok`. Nothing is imported and nothing of ours is moved aside; the deletion arrives as an entry of its own. Except for a note the entry would have moved: that deletion is at the new path, where the note never got to, and one with no id has nothing else to find it by — so the note whose file is gone is let go of there and then. Only "it is not there" is survivable — a refused or broken read is a batch nobody has decided, and moving the cursor past it would skip the changes for good.
...
- **A path can be reused.** A note is identified by `remoteId`; the path is a fallback, and it is what lets a note created here be recognised when its own first push comes back. The one note the fallback must not claim is one whose own remote copy is alive elsewhere in the batch: that note has been moved and the path merely reused, and claiming it would hand two notes each other's contents. Nor, found by a model of two devices syncing at random over every provider (`tests/sync/overProviders.test.ts`), four more, each of which hands one device's file to a note that is not its note:
  ...
  - *a note with a file of its own that the batch does not say is gone* — unmentioned because unchanged, so this is a different file that took the name (a full scan, which never says what was removed, is the exception);
  ...
  A deletion is matched the other way about — by id when it has one, and by path only when it has not, since a deletion whose id we have never seen is about a file that is not ours. By path, it is not about a note the user has renamed *onto* the path (often this is this device's own delete of the note that had the name, coming back) but about one renamed *from* it, whose file was there — ahead of a note at the path that has never had a file.
