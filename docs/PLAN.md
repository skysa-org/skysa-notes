...
- **One note per path, once the batch has been applied.** A note created here and never pushed can be sitting exactly where a remote one is about to land — two devices both writing an `Untitled.md` offline is the ordinary way to get there. Left alone, the two share a path: the sidebar shows one row twice, and the queued write for ours eventually lands on the other one's file, after which both rows carry one remote id and overwrite each other for ever. So ours moves aside first, under the name a conflict copy would get, because that is what it is. A note that *has* been pushed is left where it is: its file is elsewhere by now and its own entry is what moves it, so renaming it here would alarm the user about a note that is not in conflict. That leaves duplicates possible *during* a batch, which is why the store must not put a unique index on the path or make room by deleting what is there.
