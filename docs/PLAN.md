...
- **A file can be gone by the time we read it.** `changes` and `read` are separate round trips on every provider, so a file the feed named can be deleted in between — the other device editing a note and then deleting it inside one cursor window is enough. The entry is skipped rather than thrown on: unwinding the pull leaves the cursor where it was, so the next attempt fetches the same batch and dies the same way, and push never runs either because `sync` stops on a pull that is not `ok`. Nothing is imported and nothing of ours is moved aside; the deletion arrives as an entry of its own. Except for a note held by that very file when the batch also reports a deletion at the entry's path: that deletion may be at a path the note never got to, since the entry would have moved it there, and one with no id has nothing else to find it by — so the note whose file is gone is let go of there and then. Without such a deletion, a read's "not found" alone does not let go of a note, since nothing would ever bring it back if the read was wrong. Only "it is not there" is survivable — a refused or broken read is a batch nobody has decided, and moving the cursor past it would skip the changes for good.
