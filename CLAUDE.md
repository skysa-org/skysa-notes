...
Local-first markdown notes PWA syncing to an app-owned folder on Google Drive, OneDrive, or Dropbox (WebDAV is deferred indefinitely — `docs/ARCHITECTURE.md` §5.4). Read `docs/ARCHITECTURE.md` before any non-trivial task; it is the source of truth for the architecture and the decisions behind it; open work is in GitHub issues. If a task requires deviating from it, say so and propose the edit to `docs/ARCHITECTURE.md` in the same PR.
...
- Identity: there is none, and that is the design, not a gap — each connected storage account is its own silo, reached by a credential the device holds (docs/ARCHITECTURE.md §6, "Per-connection credentials"). Sign-in separate from storage was Phase 9 and was **dropped** (2026-09-18): identity and data are coupled on purpose, so this is decided against rather than deferred. That closes the `arctic` question by not asking it. Do not add an identity layer, a `users` subject, or a second OAuth flow; if a task seems to need one, say so rather than building it. Storage OAuth, which is a different thing, is hand-rolled over Web Crypto and installs no dependency. Validation: zod. Tests: Vitest.
...
- **Three kinds of secret, three homes, and one of them is a knowing compromise.**
  ...
  3. *The per-connection credential* (`sk1_…`, docs/ARCHITECTURE.md §6) — generated in the browser, held in IndexedDB, and sent to the server **only as its SHA-256**. The server never holds the plaintext: not in D1, not in a log, not in a `Location`, a `Set-Cookie` or a body. There is a test that proves it by generating a known secret and grepping every response in the flow; do not delete it.
...
- No operator-specific or deployment-specific code in this repo. Anything one deployment needs enters only through the `EntitlementProvider` and `RateLimiter` seams passed to `createApp`, and the `ENABLED_PROVIDERS` env flag. (There is no `IdentityProvider` seam — it was for a sign-in layer that was dropped. `AUTH_MODE` survives with one legal value and refuses to boot on the other; removing it is its own change, since it is breaking for anyone who has it set — issue #118.)
...
## Working style
- Work one issue at a time, and check the change against `docs/ARCHITECTURE.md` before starting. Start in plan mode for anything touching more than one package.
- Prefer small PRs against `main`. Each PR: tests for new behavior, an amendment to `docs/ARCHITECTURE.md` if a recorded decision changes, and — if it changes anything under `apps/` or `packages/` — a changeset (`pnpm run changeset`, or `--empty` when nothing observable moved). CI fails a PR that changes a package without one. The three workspace packages share one version and one `vX.Y.Z` tag; `.changeset/README.md` says why and how a release is cut.
