 ...
 * write after seven days of Safari use without a visit. For a library with no
 * storage connected, that store is the only copy there is, and nothing inside
 * the origin can tell afterwards that it was cleared: the next open looks like
 * a new install (docs/ARCHITECTURE.md §8, "Keeping the store").
 *
 ...
 *
 * `first-note` is the first note written on a device with nothing connected:
 * the first thing here that exists nowhere else. It asks once per device, and
 * the answer stands: after a "no", a prompt on every note would be asking the
 * user to change their mind by wearing them down. `installed` is the app being installed, which in
 * Chromium is what the browser's heuristic weighs most, so a "no" from before
 * is asked again: the answer it was is no longer the answer it would be.
 */
...
export interface Keeping {
	...
	/** Ask it to keep the store, unless it already does or has been asked. */
...
export const createKeeping = (keeper: () => StorageKeeper | undefined): Keeping => {
	...
	return {
		...
		ask: async (db, occasion) => {
			...
			if (occasion === 'first-note' && (await db.prefs.get(KEEP_ASKED_KEY)) !== undefined) {
				set('not-kept');
				return;
			}
			...
			set((await storage.persist().catch(() => false)) ? 'kept' : 'not-kept');
		},
	};
};
