...
const providerList = z
  ...
  .pipe(
    z.array(z.enum(PROVIDER_KINDS)).nonempty('ENABLED_PROVIDERS must name at least one provider'),
  )
 ...
 */
function crossFieldIssues(raw: Record<string, unknown>): string[] {
  const issues: string[] = []

  const providers = rawEnvSchema.shape.ENABLED_PROVIDERS.safeParse(raw.ENABLED_PROVIDERS)
  if (providers.success) {
    for (const provider of providers.data) {
      if (provider === 'webdav') continue
      for (const key of OAUTH_ENV_KEYS[provider]) {
        if (!raw[key]) {
          issues.push(`${key}: required because ENABLED_PROVIDERS includes "${provider}"`)
        }
      }
    }
  }

  if (raw.AUTH_MODE === 'account-first') {
...
export function parseEnv(raw: unknown): AppConfig {
  ...
  if (env.ENABLED_PROVIDERS.includes('gdrive')) {
    oauth.gdrive = oauthCredentials.parse({
      clientId: env.GOOGLE_CLIENT_ID,
      clientSecret: env.GOOGLE_CLIENT_SECRET,
    })
  }
  if (env.ENABLED_PROVIDERS.includes('onedrive')) {
    oauth.onedrive = {
  ...
  if (env.ENABLED_PROVIDERS.includes('dropbox')) {
    oauth.dropbox = oauthCredentials.parse({
